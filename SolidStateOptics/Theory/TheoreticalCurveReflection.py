import numpy as np
import scipy.constants as const
import matplotlib.pyplot as plt


import os, sys
sys.path.insert(1, "/".join(os.path.realpath(__file__).split("/")[0:-2]))
from macroswriter import writeLatexMacro  # Import macroswriter module

# Konstanten
e = const.e
m_eff = 1*const.m_e
epsilon_0 = const.epsilon_0
print(const.m_e)
N_Dichte = 5.9*10**28
tau1 = 0.1*10**-12
tau2 = 0.01*10**-12
tau3 = 0.001*10**-12
tau4 = 0.0001*10**-12
tau5 = 0.00001*10**-12
c=const.c
d = 500*10**-5 #dicke si undoped
k_max = 1*10**8
k_max_semicon = 1400000
epsilon_inf = 10.3648
omega_LO = 292*100 
omega_TO = 268*100 
gamma = 2.5*100
N_DichteSemicon = 1.05*10**24 
m_eff_Semicon = 0.067*const.m_e



def calculateReflectivity(d, tau, k):
    omega = c*k
    omega_p = np.sqrt(N_Dichte*e**2/(m_eff*epsilon_0))
    epsilon = 1-omega_p**2*tau**2/(1+omega**2*tau**2)+1j*omega_p**2*tau/omega/(1+omega**2*tau**2)
    N = np.sqrt(epsilon)

    #Gleichung 1.60 skript
    expo = 1j*2*omega*N*d/c
    r=(np.exp(expo)-1)*(1-N)/(np.exp(expo)*(1-N)-(1+N))
    R_FabryPerot = np.abs(r)**2
    return R_FabryPerot

# Berechnung der Plasmafrequenz und des zugehörigen Wellenvektors für den ersten Plot
omega_plasma = np.sqrt(N_Dichte * e**2 / (m_eff * epsilon_0))
k_plasma = omega_plasma / c
def plotReflectivity(d, tau1, tau2, tau3, tau4, tau5, k_max):
    fig, ax = plt.subplots()
    ax.grid()
    
    k = np.linspace(0, k_max, 10000)

    taus = np.logspace(np.log10(tau1), np.log10(tau5), 50)  # Generate 50 tau values logarithmically spaced
    for tau in taus:
        ax.plot(k*10**-8, calculateReflectivity(d, tau, k), color='gray', linewidth=0.5, alpha=0.7)
        
    ax.plot(k*10**-8, calculateReflectivity(d, tau5, k), linewidth=3, label=r'$10^{-17}s$')
    ax.plot(k*10**-8, calculateReflectivity(d, tau4, k), linewidth=3, label=r'$10^{-16}s$')
    ax.plot(k*10**-8, calculateReflectivity(d, tau3, k), linewidth=3, label=r'$10^{-15}s$')
    ax.plot(k*10**-8, calculateReflectivity(d, tau2, k), linewidth=3, label=r'$10^{-14}s$')
    ax.plot(k*10**-8, calculateReflectivity(d, tau1, k), linewidth=3, label=r'$10^{-13}s$')
    #ax.set_xticks([0, 0.25, 0.5, 0.75, 1], [r'$\Gamma$', 'X', 'M', 'Y', r'$\Gamma$'], size=14)

    ax.set_ylabel('Reflectivity R', fontsize=14)
    ax.set_xlabel(r'wavenumber $\nu$ / ' + r'$10^{8}\,m^{-1}$', fontsize=14)
    ax.tick_params(axis='both', direction='in', which='both', top=True, right=True, labelsize=14)
    ax.set_xlim(left=-0.01*k_max*10**-8, right=k_max*10**-8)
    ax.set_ylim(bottom=-0.05, top=1.05)
    
    ax.axvline(x=k_plasma*10**-8, color='black', linestyle='--', label=r'$\nu_{plasma}$')
    ax.legend(fontsize=14)

    plt.savefig('Paper/Images/theoretical_reflectivity.png', dpi=400)
    from PIL import Image
    Image.open("Paper/Images/theoretical_reflectivity.png").show()
    plt.clf()
    plt.close()

plotReflectivity(d, tau1, tau2, tau3, tau4, tau5, k_max)


writeLatexMacro('N_Dichte',np.round(N_Dichte,4))